...
Процедура ЗагрузитьФайл(Команда)
	...
	ПараметрыЗагрузки.Вставить("Заголовки", Заголовки);
	
	Если КудаСохранять = 0 Тогда
		
		Если Не ПустаяСтрока(ПустаяСтрока(ПутьНаКлиенте)) Тогда 
			ПараметрыЗагрузки.Вставить("ПутьДляСохранения", ПутьНаКлиенте);
		КонецЕсли;
		
		Результат = ПолучениеФайловИзИнтернетаКлиент.СкачатьФайлНаКлиенте(АдресДляЗагрузки, ПараметрыЗагрузки);
	...
	ИначеЕсли КудаСохранять = 1 Тогда
		...
		Если Не ПустаяСтрока(ПустаяСтрока(ПутьНаСервере)) Тогда 
			ПараметрыЗагрузки.Вставить("ПутьДляСохранения", ПутьНаСервере);
		КонецЕсли;
		
		Результат = СкачатьФайлНаСервере(АдресДляЗагрузки, ПараметрыЗагрузки);
